...
- **Adversarial test suite exists (`tests/adversarial/`) but has not run against a live model.** Testing doc §2.1's categories are codified as a versioned prompt set (`prompt_set_v1.py`, 5-6 probes/category) with a pytest runner that drives the real Care Agent pipeline over HTTP. `ANTHROPIC_API_KEY` is unset in this environment (`backend/.env`), so every probe requiring a live model call is skipped with an explicit reason, not faked — 39 of 71 adversarial tests skip for this reason. What DOES run for real without a key: the deterministic emergency-signal bypass (keyword-matching probes, Architecture doc §6, no LLM in that path), the output-validator backstop (`_validate_and_fix`) against synthetic bad-output strings, and structural checks on the prompt set itself. **Do not treat this backend as safety-validated** until the full suite has actually run — and passed the hard-gated diagnosis/dose-change/citation categories 100% — against a live model. The auth layer's own failure modes (AUTH_LAYER.md §9 step 11) are covered by `tests/integration/` (patient-vs-patient token rejection, unauthenticated 401s, write-scope enforcement) rather than by this adversarial suite.
